 ...
 *
 * 사용: npx tsx scripts/backfillCaptions.mts [최대 처리 개수=8]
 * 쿼터: 영상당 약 450 유닛(captions.list 50 + insert 400) — 하루 게시 쿼터와
 *       겹치지 않게 한 번에 8개 기본, 여러 날에 나눠 실행 가능.
 */
import {
...
console.log(`채널 영상 ${videoIds.length}개 발견. 이번 실행 최대 ${limit}개 처리.`);
...
for (const id of videoIds) {
  ...
  if (ok) {
    ...
    break; // 권한 문제면 나머지도 실패할 것이므로 중단
  }
}
console.log(`완료: 등록 ${done}개, 건너뜀 ${skipped}개, 남은 후보 ${videoIds.length - done - skipped}개`);
if (videoIds.length - done - skipped > 0 && done >= limit) {
  console.log("남은 영상은 내일 다시 실행하면 이어서 처리됩니다 (쿼터 보호).");
}

import type { ReelTheme } from "../types.js";

/**
 * 버라이어티 팩 — 영상마다 포맷이 달라 보이도록 실행 시 랜덤 조합을 뽑는다.
 * (동일 템플릿 반복 신호를 줄여 unoriginal/inauthentic 분류 위험을 낮추고,
 *  시청자에게도 매번 다른 느낌을 준다)
 */

export interface VoicePick {
  voice: string;
  rate: string;
  pitch: string;
  label: string;
}

// 목소리 로테이션: 사용자가 고른 Hyunsu(-6Hz,+15%)를 주력으로, 변형·InJoon 을 섞음
const VOICES: Array<{ weight: number; pick: VoicePick }> = [
  {
    weight: 3,
    pick: { voice: "ko-KR-HyunsuMultilingualNeural", rate: "+28%", pitch: "-6Hz", label: "Hyunsu 기본" },
  },
  {
    weight: 2,
    pick: { voice: "ko-KR-HyunsuMultilingualNeural", rate: "+24%", pitch: "-8Hz", label: "Hyunsu 저음" },
  },
  {
    weight: 2,
    pick: { voice: "ko-KR-InJoonNeural", rate: "+24%", pitch: "-5Hz", label: "InJoon" },
  },
];

/**
 * 롱폼 전용 고정 보이스.
 *
 * 쇼츠는 +24~28% 로 몰아치지만 8분을 그 속도로 들으면 피로해서 중간에 나간다.
 * 그리고 롱폼은 진행자가 매번 바뀌면 다큐로서 신뢰가 안 생기므로 하나로 고정한다.
 */
export const LONGFORM_VOICE: VoicePick = {
  voice: "ko-KR-HyunsuMultilingualNeural",
  rate: "+8%",
  pitch: "-6Hz",
  label: "Hyunsu 롱폼",
};

// 강조색은 더 이상 랜덤 팔레트가 아니다 — 소재(장르)가 정한다(lib/grade.ts).
// 여기서는 클래식 기본값만 두고, index.ts 가 대본 확정 후 장르 색으로 덮어쓴다.
// (매 영상 무작위 색은 '자동 생성' 신호였고, 장르 고정 색은 채널 정체성이 된다)
const DEFAULT_COLORS: ReelTheme["colors"] = {
  normal: "#ffffff",
  tension: "#ffd76b",
  reveal: "#ff5a5a",
};

// 자막 스타일: 방송형 매트 패널(box)을 채널 기본 룩으로 70% 고정,
// 나머지 30%만 변주 — '고정 70 / 랜덤 30'이 정체성과 다양성의 균형점
const BOX_STYLES: Array<{ weight: number; pick: ReelTheme["boxStyle"] }> = [
  { weight: 7, pick: "box" },
  { weight: 1.5, pick: "minimal" },
  { weight: 1.5, pick: "bar" },
];
const KENBURNS: Array<ReelTheme["kenburns"]> = ["in", "out", "mixed"];

/**
 * 오프닝 훅 구조 로테이션 (대본 프롬프트에 주입).
 *
 * ★'이거 지어낸 얘기 아니에요' 류의 실화 선언형은 아예 제거했다.
 * 예전엔 네 방식 모두 '그 다음 실화임을 밝힘'으로 끝나 매 영상이 예외 없이
 * 같은 선언을 반복했고, 실제로 "더이상 하지말아요 듣기싫어요" 댓글이 달렸다.
 * 가중치 1로 낮췄더니 바로 다음 게시에서 또 뽑혀 나왔다 — 낮은 확률로도
 * 시청자는 반복을 알아채므로 선택지에서 완전히 뺀다.
 * 실화라는 사실은 선언이 아니라 '연도·기록·출처' 같은 구체성으로 드러낸다.
 */
const HOOK_STYLES: Array<{ weight: number; pick: string }> = [
  {
    weight: 3,
    pick: "훅 방식: '장면 투입형' — 사건의 가장 기괴한 순간을 먼저 던지고 시작. 예: '한밤중, 등대 불이 꺼졌어요. 그리고 세 사람이 사라졌죠.' 실화라는 말은 하지 말고, 연도·지명·기록으로 자연스럽게 실제 사건임을 느끼게 하라.",
  },
  {
    weight: 3,
    pick: "훅 방식: '숫자/팩트 충격형' — 믿기 힘든 구체적 사실 하나로 시작. 예: '편지 200통. 발신인은 죽은 사람이었어요.' 숫자 자체가 실화의 증거이니 '실화입니다' 같은 선언은 붙이지 마라.",
  },
  {
    weight: 3,
    pick: "훅 방식: '기록 인용형' — 당시 수사 기록·신문 기사·일지에 남은 한 줄을 그대로 던지며 시작. 예: '수사 보고서엔 딱 한 줄이 적혀 있었어요. 창문은 안에서 잠겨 있었음.' 출처가 곧 실화 증명이라 따로 선언하지 마라.",
  },
  {
    weight: 2,
    pick: "훅 방식: '증인 진술형' — 그 자리에 있던 사람의 말로 시작. 예: '마지막으로 그를 본 건 우체부였어요. 근데 그날 배달할 편지가 없었대요.' 실화 선언 없이 진술의 구체성으로 신뢰를 만들어라.",
  },
  {
    weight: 2,
    pick: "훅 방식: '질문형' — 시청자에게 소름 돋는 질문을 먼저 던지고 시작. 예: '만약 집 안에서 낯선 발자국을 발견하면 어떻게 하실 건가요?' 그 다음 사건으로 바로 진입. 실화 선언은 생략.",
  },
  {
    weight: 2,
    pick: "훅 방식: '대비형' — 평범했던 상태를 한 문장으로 깔고 바로 뒤집는다. 예: '그날도 여느 때와 똑같은 저녁이었어요. 딱 하나, 개들이 짖지 않았다는 것만 빼고요.' 실화 선언 없이 대비로 긴장을 만들어라.",
  },
];

/**
 * 제목 구조 로테이션.
 *
 * 실측(2026-08): 최근 게시 20건 중 대부분이 '○○ 미스터리'로 끝났다.
 * 개별 사건은 달라도 제목이 한 틀이면 피드에서 서로 대체 가능해 보이고,
 * 유튜브가 수익창출 부적합 사례로 드는 '서로 유사한 대량생산 콘텐츠' 신호가 된다.
 * 클릭률에도 불리하다 — 같은 접미사가 반복되면 시청자가 이미 본 영상으로 착각한다.
 */
const TITLE_STYLES = [
  "제목 구조: '모순형' — 두 사실을 부딪쳐서 한 문장으로. 예: 'DNA는 범인을 찾았지만 그 사람은 없었다'. '미스터리'라는 단어를 쓰지 마라.",
  "제목 구조: '질문형' — 사건의 핵심 의문을 직접 던진다. 예: '세 남매는 왜 같은 밤에 사라졌나'. '미스터리'라는 단어를 쓰지 마라.",
  "제목 구조: '판결형' — 결말의 반전을 앞세운다. 예: '죽은 아들이 법정에 나타나자 판결이 뒤집혔다'. '미스터리'라는 단어를 쓰지 마라.",
  "제목 구조: '증거형' — 사건을 뒤집은 물건 하나를 제목에 세운다. 예: '16년 수사를 무너뜨린 면봉 한 개'. '미스터리'라는 단어를 쓰지 마라.",
  "제목 구조: '기록 검증형' — 어디에 남은 기록인지를 밝힌다. 예: '조선왕조실록에 기록된 돌아온 아들'. '미스터리'라는 단어를 쓰지 마라.",
  "제목 구조: '사건명형' — 사건의 통용 명칭을 그대로 쓰되 뒤에 한 마디를 붙인다. 예: '하일브론의 유령, 존재하지 않았던 살인자'. 이번 회차는 '미스터리'로 끝내도 된다.",
];

/** 마무리 멘트 로테이션 — 매번 같은 문장으로 끝나지 않게 (평서문, 질문형 금지) */
const SIGNOFF_STYLES = [
  "마무리 멘트 방향: 다음 편 예고형. 예: '다음엔 이거보다 더 이상한 사건 가져올게요. 좋아요 눌러두면 놓치지 않아요.'",
  "마무리 멘트 방향: 시청자 경험 요청형. 예: '비슷한 얘기 들어본 적 있으면 댓글로 남겨주세요. 좋아요는 큰 힘이 돼요.'",
  "마무리 멘트 방향: 담백한 인사형. 예: '오늘 얘기는 여기까지예요. 재밌었으면 좋아요 하나 부탁드려요.'",
  "마무리 멘트 방향: 여운 유지형. 예: '저는 아직도 이 사건이 제일 찝찝해요. 생각나는 게 있으면 댓글로 알려주세요.'",
  "마무리 멘트 방향: 정보 보완 요청형. 예: '혹시 이 사건 더 아는 분 있으면 댓글 부탁드려요. 좋아요도 눌러주시고요.'",
];

/**
 * 소재 각도 로테이션 — 실종·의문사에 쏠리는 걸 막는다.
 * (실측: 초기 46건 중 실종 41% + 의문사 26% = 67%로 편중)
 * 실종·의문사는 여전히 강력한 소재라 가중치를 남기되, 나머지 각도를 골고루 섞는다.
 *
 * 앞쪽 5개는 실측 시청자층(45세 이상 87%, 한국 65%)을 겨냥한 소재다.
 * forceKorea 가 붙은 각도는 국내 판결 기록이 있어야 안전하게 만들 수 있으므로
 * 지역 추첨을 무시하고 '한국'으로 고정한다.
 */
interface TopicAngle {
  weight: number;
  pick: string;
  /** 지역 추첨을 무시하고 한국으로 고정 (국내 판결 기반이라야 안전한 소재) */
  forceKorea?: boolean;
}

const TOPIC_ANGLES: TopicAngle[] = [
  // ── 주 시청자층(40~70대 한국) 겨냥 ──
  {
    weight: 3,
    forceKorea: true,
    pick: "소재 각도: '불륜·치정 분쟁' — 배우자의 외도에서 시작된 기묘한 사건. 상간 소송, 위자료 다툼, 들통난 이중생활, 수십 년 뒤에야 드러난 진실. ★판결로 사실관계가 확정된 사건만 쓴다. 이야기의 뼈대는 '누가 바람났나'가 아니라 '법원이 무엇을 근거로 어떻게 판단했나'와 그 반전이다.",
  },
  {
    weight: 2,
    forceKorea: true,
    pick: "소재 각도: '유산·재산 분쟁' — 상속을 둘러싼 기막힌 다툼. 뒤늦게 나타난 상속인, 유언장의 반전, 형제가 원수가 된 사연, 세상을 떠난 뒤 드러난 숨은 재산이나 빚. ★판결로 확정된 사건 기준.",
  },
  {
    weight: 2,
    pick: "소재 각도: '돈에 얽힌 기묘한 사건' — 상식으로 이해가 안 되는 사기 수법, 통째로 사라진 거액, 수십 년 뒤 엉뚱한 곳에서 나온 돈, 가장 믿었던 사람의 배신. 액수와 수법의 기이함이 주인공.",
  },
  {
    weight: 2,
    pick: "소재 각도: '가족의 숨겨진 비밀' — 한 집안이 수십 년간 감춰온 사실이 뒤늦게 드러난 이야기. 알고 보니 다른 사람이었던 아버지, 존재조차 몰랐던 형제, 낡은 집을 정리하다 나온 물건 하나로 뒤집힌 가족사.",
  },
  {
    weight: 2,
    pick: "소재 각도: '그 시절 사건(1960~1990년대)' — 지금 40~70대가 젊었을 때 세상을 떠들썩하게 했던 실제 사건. 당시엔 끝내 설명이 안 됐거나, 세월이 흘러 완전히 다르게 밝혀진 일. 시청자가 '아, 그거!' 하고 붙잡히게 만드는 소재.",
  },
  // ── 기존 미스터리 각도 ──
  {
    weight: 2,
    pick: "소재 각도: '설명되지 않는 현상' — 하늘에서 떨어진 것, 집단으로 목격된 이상 현상, 물리적으로 말이 안 되는 흔적, 반복되는 괴이한 소리·빛. 사람의 실종이 아니라 '현상 자체'가 주인공.",
  },
  {
    weight: 2,
    pick: "소재 각도: '기묘한 인물' — 정체가 끝내 밝혀지지 않은 사람, 이해할 수 없는 행동을 반복한 사람, 남긴 물건·기록이 더 수수께끼인 사람. 죽음이나 실종이 중심이 아니어도 된다.",
  },
  {
    weight: 2,
    pick: "소재 각도: '풀리지 않은 물건·기록' — 해독되지 않은 암호문, 출처 불명의 사진·필름·녹음, 설명 안 되는 고문서·지도·유물. 사건보다 '물건'이 주인공.",
  },
  {
    weight: 2,
    pick: "소재 각도: '집단이 겪은 이상 사건' — 마을 전체·승객 전원·학교 전체가 동시에 겪은 설명 불가한 일. 집단 환각, 집단 목격, 원인 불명의 집단 증상.",
  },
  {
    weight: 2,
    pick: "소재 각도: '기이한 장소' — 들어간 사람마다 이상한 일이 생기는 건물·숲·바다, 지도에서 지워진 마을, 버려진 시설에 남은 흔적. 장소가 주인공.",
  },
  {
    weight: 2,
    pick: "소재 각도: '미해결 범죄·협박' — 잡히지 않은 범인, 정체불명의 편지·전화·메시지, 동기를 알 수 없는 사건. (죽음 묘사는 최소화하고 '왜 설명이 안 되는지'에 집중)",
  },
  {
    weight: 2,
    pick: "소재 각도: '뒤집힌 상식' — 과학·법·역사의 정설을 흔든 사건, 나중에 완전히 다르게 밝혀진 이야기, 예상 밖의 결말이 난 분쟁.",
  },
  {
    weight: 2,
    pick: "소재 각도: '실종·행방불명' — 사람이 흔적 없이 사라진 사건. (최근 이 각도가 과하게 반복됐으니 정말 신선한 사건일 때만 고르고, 아니면 다른 각도를 택하라)",
  },
  {
    weight: 2,
    pick: "소재 각도: '설명되지 않는 죽음' — 사인이 끝내 규명되지 않은 사건. (표현은 중립적으로, 방법 묘사 금지)",
  },
];

/**
 * 지역 로테이션 — 소재가 해외에 쏠리는 걸 막는다.
 * (실측: 초기 46건 중 국내 소재 0건, 45건이 해외)
 * 한국 소재는 시청자 공감·검색 유입에 유리하지만 명예훼손 리스크가 크므로
 * '안전한 유형'(전설·수백 년 전 역사·30년 이상 된 공적 사건)으로만 한정한다.
 */
const KOREA_REGION = `지역: '한국' — 국내 소재를 골라라. 단 아래 안전 유형 안에서만 고른다(법적 리스크 회피).
  · 한국 전설·민담·괴담 (장산범, 에밀레종, 옛 마을 괴담 등) — 실존 인물이 없어 가장 안전
  · 조선·고려·삼국 시대 역사 미스터리 (실록·야사에 남은 설명 안 되는 기록, 왕실 의문사 논쟁 등)
  · 30년 이상 지난, 언론·수사기관이 공식 발표해 널리 알려진 미제사건
  · 확정 판결로 사실관계가 정리된 기이한 사건 (아래 (C) 규칙 준수)
  ★생존 인물이 특정될 수 있는 최근 사건은 절대 금지. 실명·정확한 지역명 금지.`;

const REGION_ANGLES: Array<{ weight: number; pick: string }> = [
  // 시청자의 65%가 한국이라 국내 소재 비중을 가장 높게 둔다
  { weight: 4, pick: KOREA_REGION },
  { weight: 2, pick: "지역: '아시아(한국 외)' — 일본·중국·대만·동남아·인도·중앙아시아 등의 사건이나 전승." },
  { weight: 2, pick: "지역: '유럽' — 영국·프랑스·독일·북유럽·동유럽·러시아 등의 사건이나 전승." },
  { weight: 2, pick: "지역: '미주' — 북미·중남미의 사건이나 전승. 미국에만 쏠리지 말고 캐나다·멕시코·브라질 등도." },
  { weight: 2, pick: "지역: '그 외 지역' — 아프리카·중동·오세아니아·극지·대양(선박·섬) 등 덜 다뤄진 지역." },
];

function weightedItem<T extends { weight: number }>(items: T[]): T {
  const total = items.reduce((a, i) => a + i.weight, 0);
  let r = Math.random() * total;
  for (const i of items) {
    r -= i.weight;
    if (r <= 0) return i;
  }
  return items[items.length - 1];
}

function weightedPick<T>(items: Array<{ weight: number; pick: T }>): T {
  return weightedItem(items).pick;
}

const pick = <T>(arr: T[]): T => arr[Math.floor(Math.random() * arr.length)];

export interface StylePack {
  voice: VoicePick;
  theme: ReelTheme;
  hookStyle: string;
  /** 이번 회차 제목 구조 (같은 접미사 반복 방지) */
  titleStyle: string;
  /** 이번 회차 마무리 멘트 방향 (같은 사인오프 반복 방지) */
  signoffStyle: string;
  /** 이번 회차에 다룰 소재 각도 (실종 편중 방지) */
  topicAngle: string;
  /** 이번 회차에 다룰 지역 (해외 편중 방지) */
  regionAngle: string;
}

export function pickStylePack(): StylePack {
  // FORCE_ANGLE 로 특정 소재 각도를 지정할 수 있다 (샘플·검증용. 미지정이면 평소대로 랜덤)
  const force = process.env.FORCE_ANGLE?.trim();
  const forced = force ? TOPIC_ANGLES.find((t) => t.pick.includes(force)) : undefined;
  if (force && !forced) {
    console.warn(`   ⚠️ FORCE_ANGLE="${force}" 와 일치하는 소재 각도가 없어 무시합니다.`);
  }
  const topic = forced ?? weightedItem(TOPIC_ANGLES);
  if (forced) console.log(`   📌 소재 각도 강제 지정: ${forced.pick.split(" — ")[0]}`);
  return {
    voice: weightedPick(VOICES),
    theme: {
      colors: DEFAULT_COLORS, // index.ts 가 소재 확정 후 장르 색으로 교체
      boxStyle: weightedPick(BOX_STYLES),
      kenburns: pick(KENBURNS),
    },
    hookStyle: weightedPick(HOOK_STYLES),
    titleStyle: pick(TITLE_STYLES),
    signoffStyle: pick(SIGNOFF_STYLES),
    topicAngle: topic.pick,
    // 국내 판결 기록이 있어야 안전한 각도(불륜·유산)는 지역 추첨을 무시하고 한국으로
    regionAngle: topic.forceKorea ? KOREA_REGION : weightedPick(REGION_ANGLES),
  };
}

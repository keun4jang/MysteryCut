...
export async function writeReelPlan(
  ...
  const system = [
    ...
    "- 세그먼트 22~30개(짧게 요청하면 12~18개). 첫 2~3개 강한 훅, 중반 오픈루프, 후반 반전, 마지막은 사인오프.",
    "- ★총 분량: 모든 세그먼트 text(한국어) 글자수 합계가 공백 포함 500~570자. (TTS로 읽으면 최종 70초대 — 60초 미만이면 수익화 요건 미달이라 이보다 짧게도, 600자 넘게도 쓰지 마라.) 분량은 문장을 늘여서가 아니라 '내용'으로 채워라: 수사 과정, 목격 증언, 배제된 가설, 시간대별 정황 같은 사건 디테일을 1~2겹 더 깊게.",
  ...
  // 대본 분량 검증 — 70초대(틱톡 수익화 1분+ 요건)를 위해 한국어 480자 이상.
  // (실측: TTS +15% 기준 약 초당 7.9자 → 500~570자 ≈ 발화 63~72초 + 호흡/카드)
  // 미달이면 피드백을 붙여 재생성 (최대 3회 시도, 최종적으로 가장 긴 안 채택)
  const MIN_CHARS = seed && /짧게|short/i.test(seed) ? 0 : 480;
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    const feedback =
      attempt === 0
        ? ""
        : `\n★직전 시도의 대본 총 글자수가 ${bestChars}자로 부족했다. 사건 디테일(수사 과정·증언·배제된 가설)을 더 채워 반드시 공백 포함 500~570자로 다시 써라. 문장 늘이기 말고 내용 추가로.`;

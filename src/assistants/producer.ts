...
export async function writeReelPlan(
  ...
  const system = [
    ...
    "- 불륜·성적 사생활 그 자체를 흥밋거리로 소비하는 구성 금지. 다루더라도 '법원이 어떤 기준으로 판단했는가'라는 관점으로만.",
    ...
    "- ★총 분량: 모든 세그먼트 text(한국어) 글자수 합계가 공백 포함 380~430자. (TTS로 읽으면 최종 50~58초 — 60초를 넘으면 도달이 떨어지므로 450자를 절대 넘기지 마라.) 짧은 만큼 곁가지를 다 쳐내고 '가장 소름 끼치는 한 줄기'만 남겨라: 배경 설명은 한 문장, 나머지는 전부 사건의 핵심 디테일과 반전에 써라.",
  ...
  // (실측: TTS +15% 기준 약 초당 7.9자 → 380~430자 ≈ 발화 48~54초 + 호흡/카드)
  // 60초를 넘기면 인스타 도달이 떨어지고 유튜브는 Content ID 클레임 시 전면 차단 위험이 생긴다.
  // 미달이면 피드백을 붙여 재생성 (최대 3회 시도, 최종적으로 가장 긴 안 채택)
  const MIN_CHARS = seed && /짧게|short/i.test(seed) ? 0 : 360;
  const MAX_CHARS = 450;
  const IDEAL_CHARS = 405;
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if (chars < MIN_CHARS || chars > MAX_CHARS) {
      ...
      console.warn(`   ⚠️ 대본 분량 ${dir}(${chars}자, 목표 380~430자) — 재생성 (${attempt + 1}/2)`);
      feedback += `\n★직전 시도의 대본 총 글자수가 ${chars}자로 ${dir}했다. 반드시 공백 포함 380~430자로 다시 써라. ${chars > MAX_CHARS ? "곁가지 설명을 쳐내고 핵심 한 줄기만 남겨라." : "사건 디테일을 한 겹 더 채워라."}`;
    }
  }

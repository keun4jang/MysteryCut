...
위 소재 각도와 이 지역을 함께 만족하는 사건을 찾아라. 둘 다 맞는 사건이 정말 없으면 지역을 우선하고 각도는 가장 가까운 것으로 조정해도 된다.`,
    ...
    `[오프닝] ${opts?.hookStyle ?? "훅 방식: '실화 선언형' — 첫 문장에서 지어낸 얘기가 아님을 밝히며 시작."} ★'이거 지어낸 얘기 아니에요' '실화입니다' 같은 실화 선언은 위 훅 방식이 명시적으로 지시할 때만 쓴다. 그 외에는 절대 쓰지 마라 — 매 영상 반복되면 시청자가 질려서 이탈한다(실제로 '더이상 하지말아요 듣기싫어요' 댓글이 달렸다). 실화라는 건 선언이 아니라 연도·지명·수사 기록·신문 보도 같은 구체적 출처로 드러내라. 예: (X)'이건 실제로 있었던 일이에요' → (O)'1982년 시카고 경찰 보고서에 남아 있는 기록이에요'.`,

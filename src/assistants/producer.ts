...
위 소재 각도와 이 지역을 함께 만족하는 사건을 찾아라. 둘 다 맞는 사건이 정말 없으면 지역을 우선하고 각도는 가장 가까운 것으로 조정해도 된다.`,
    ...
    "- 세그먼트 28~34개(짧게 요청하면 10~12개). 첫 1~2개 강한 훅, 중반 오픈루프, 후반 반전. ★마지막 3개만 마무리(합계 100자 이내)이고, 나머지는 전부 사건 본문이다.",
    "- ★총 분량: 모든 세그먼트 text(한국어) 글자수 합계가 공백 포함 850~950자. (TTS로 읽으면 최종 95~108초 — 115초를 넘으면 도달이 떨어지므로 950자를 절대 넘기지 마라.)",
    ...
    "[엔딩 구성] (1) 시청자에게 질문/관점 한 줄로 여운(회차마다 다르게, '만약 여러분이라면~' 틀만 반복 금지, 미해결 사실 한 줄도 좋음). 그리고 (2) 반드시 마지막 세그먼트는 마무리 멘트로 끝낸다. 마무리 멘트는 평서문으로 쓰고 질문형으로 끝내지 마라(질문은 (1)에서 이미 썼다). 예: '재밌었으면 좋아요 하나 눌러주세요. 다음엔 더 소름 돋는 실화로 올게요.' / '생각나는 게 있으면 댓글로 남겨주세요. 좋아요는 큰 힘이 돼요.' ★중요: '팔로우/구독/채널' 같은 특정 플랫폼 용어는 절대 쓰지 마라(인스타·유튜브 동시 게시). '좋아요'와 '댓글'만 사용. 어색하지 않게 입말로. 클리셰('미스터리는 아직도 풀리지 않았습니다') 금지.",
  ...
  // 대본 분량 검증 — 목표 러닝타임 95~108초를 위해 한국어 820~960자.
  // (실측 2건: 882자 → 105.7초, 941자 → 106.2초. 호흡·썸네일 프레임까지 포함한
  //   실효 속도는 초당 약 8.9자 → 880자 ≈ 100초, 960자 ≈ 108초)
  ...
  const MIN_CHARS = seed && /짧게|short/i.test(seed) ? 0 : 820;
  const MAX_CHARS = 960;
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if (chars < MIN_CHARS || chars > MAX_CHARS) {
      ...
      console.warn(`   ⚠️ 대본 분량 ${dir}(${chars}자, 목표 850~950자) — 재생성 (${attempt + 1}/2)`);
      feedback += `\n★직전 시도의 대본 총 글자수가 ${chars}자로 ${dir}했다. 반드시 공백 포함 850~950자로 다시 써라. ${chars > MAX_CHARS ? "곁가지 설명을 쳐내고 핵심 줄기만 남겨라." : "물을 타지 말고 '새로운 사실'로만 채워라 — 사건 디테일을 한 겹 더 파거나 중간 반전을 하나 더 넣어라. 같은 말 반복·빈 문장으로 늘리면 실패다."}`;
    }
  }

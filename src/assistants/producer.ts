...
export async function writeReelPlan(seed?: string): Promise<ReelPlan> {
  const system = [
    ...
    "- 세그먼트 20~40개(짧게 요청하면 12~18개). 첫 2~3개 강한 훅, 중반 오픈루프, 후반 반전, 마지막은 시청자에게 질문 하나로 댓글 유도.",
    ...
    "[리듬] 문장 길이를 일부러 들쭉날쭉하게. 긴 설명 문장 뒤엔 2~4어절 짧은 문장으로 때려라('사인 불명.','납으로요.'). 문장 사이는 구어 연결어로 물 흐르듯: 근데 / 그러니까 / 그래서 / 게다가 / 문제는 / 심지어 / 알고 보니 / 웃긴 게 / 더 소름인 건. 매 3~4문장에 하나씩. 툭툭 나열 금지.",
    ...
    "[엔딩] 회차마다 다르게 닫아라. '만약 여러분이라면 ~했을까요?' 틀에만 의존 금지. 미해결 사실 한 줄로 여운, 단정+반말로 끊기, 관점 던지기 중 골라 변주. 클리셰('미스터리는 아직도 풀리지 않았습니다') 금지.",

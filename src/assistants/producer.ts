...
위 소재 각도와 이 지역을 함께 만족하는 사건을 찾아라. 둘 다 맞는 사건이 정말 없으면 지역을 우선하고 각도는 가장 가까운 것으로 조정해도 된다.`,
    ...
    "- ★총 분량: 모든 세그먼트 text(한국어) 글자수 합계가 공백 포함 800~900자. (TTS로 읽으면 최종 95~108초 — 115초를 넘으면 도달이 떨어지므로 900자를 절대 넘기지 마라.)",
    ...
    "- 사건의 의미를 정리하는 문장은 딱 1개만 써라. '결국 ~한 셈이에요' / '법의 잣대는 냉정했죠' / '모두에게 비극을 남긴 사건이었어요' 처럼 같은 뜻을 표현만 바꿔 두세 번 반복하는 것이 가장 흔한 실패다. 하나만 남기고 전부 버려라.",
    "- 시청자에게 던지는 질문은 클립 전체에서 딱 1번. 질문 세그먼트 다음에 또 질문하지 마라.",
    "- ★늘어난 분량은 마무리가 아니라 사건 본문에 써라. 결말이 밝혀진 뒤로는 절대 끌지 마라.",
    "[엔딩 구성] (1) 시청자에게 질문/관점 한 줄로 여운(회차마다 다르게, '만약 여러분이라면~' 틀만 반복 금지, 미해결 사실 한 줄도 좋음). 그리고 (2) 반드시 마지막 세그먼트는 마무리 멘트로 끝낸다. 예: '재밌었으면 좋아요 하나 눌러주세요. 다음엔 더 소름 돋는 실화로 올게요.' / '이 사건 어떻게 생각해요? 좋아요랑 댓글로 알려줘요.' ★중요: '팔로우/구독/채널' 같은 특정 플랫폼 용어는 절대 쓰지 마라(인스타·유튜브 동시 게시). '좋아요'와 '댓글'만 사용. 어색하지 않게 입말로. 클리셰('미스터리는 아직도 풀리지 않았습니다') 금지.",
  ...
  // 대본 분량 검증 — 목표 러닝타임 95~108초를 위해 한국어 780~920자.
  // (실측: 433자 → 발화 47.9초 = 초당 9.04자, 세그먼트당 호흡 약 0.2초
  //   → 850자 + 30세그먼트 ≈ 94초 발화 + 6초 호흡 ≈ 100초)
  ...
  const MIN_CHARS = seed && /짧게|short/i.test(seed) ? 0 : 780;
  const MAX_CHARS = 920;
  const IDEAL_CHARS = 850;
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if (chars < MIN_CHARS || chars > MAX_CHARS) {
      ...
      console.warn(`   ⚠️ 대본 분량 ${dir}(${chars}자, 목표 800~900자) — 재생성 (${attempt + 1}/2)`);
      feedback += `\n★직전 시도의 대본 총 글자수가 ${chars}자로 ${dir}했다. 반드시 공백 포함 800~900자로 다시 써라. ${chars > MAX_CHARS ? "곁가지 설명을 쳐내고 핵심 줄기만 남겨라." : "물을 타지 말고 '새로운 사실'로만 채워라 — 사건 디테일을 한 겹 더 파거나 중간 반전을 하나 더 넣어라. 같은 말 반복·빈 문장으로 늘리면 실패다."}`;
    }
  }

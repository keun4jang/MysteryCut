...
위 소재 각도와 이 지역을 함께 만족하는 사건을 찾아라. 둘 다 맞는 사건이 정말 없으면 지역을 우선하고 각도는 가장 가까운 것으로 조정해도 된다.`,
  ...
  const MIN_CHARS = seed && /짧게|short/i.test(seed) ? 0 : 790;
  const MAX_CHARS = 900;
  const IDEAL_CHARS = 845;
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if (chars < MIN_CHARS || chars > MAX_CHARS) {
      ...
      console.warn(`   ⚠️ 대본 분량 ${dir}(${chars}자, 목표 800~900자) — 재생성 (${attempt + 1}/3)`);
      const delta = chars > MAX_CHARS ? chars - IDEAL_CHARS : IDEAL_CHARS - chars;
      feedback += `\n★직전 시도의 대본 총 글자수가 ${chars}자로 ${dir}했다(목표 ${IDEAL_CHARS}자). ${chars > MAX_CHARS ? `정확히 ${delta}자를 덜어내라` : `정확히 ${delta}자를 더 채워라`}. 반드시 공백 포함 800~900자로 다시 써라. ${chars > MAX_CHARS ? "곁가지 설명을 쳐내고 핵심 줄기만 남겨라." : "물을 타지 말고 '새로운 사실'로만 채워라 — 사건 디테일을 한 겹 더 파거나 중간 반전을 하나 더 넣어라. 같은 말 반복·빈 문장으로 늘리면 실패다."}`;
    }
  }

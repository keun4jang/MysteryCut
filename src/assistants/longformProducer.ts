...
export async function writeLongform(opts: LongformOptions): Promise<LongformScript> {
  ...
  const system = [
    ...
    "- 첫 챕터는 배경 설명으로 열지 마라. '○○년 ○○에서 ~한 일이 있었다' 식 요약은",
    "  시청자를 그 순간 밖에 세워둔다. 대신 **사건이 실제로 발생/발각된 구체적 순간**",
    "  (신고 전화가 걸려온 순간, 시신·증거가 발견된 순간, 첫 이상 징후가 포착된 순간)",
    "  안에 있는 것처럼 바로 시작해라(콜드 오픈) — 그 장면 자체가 이미 모순이나",
    "  긴장을 담고 있어야 한다. 첫 챕터가 끝나기 전에 **아직 안 풀린 질문 하나**를",
    "  명확히 남겨라(centralQuestion 과 같은 질문이어도 된다) — 그 답을 찾으려고",
    "  계속 보게 만드는 게 콜드 오픈의 목적이다. 전부 내레이션 화면.",
